	...
	if( $result)
	{
		...
		if( $num_rows > 0)
		{
			...
			while( $row= mysql_fetch_row($result))
			{
				...
				if( isset($_REQUEST['sql_extends_inserts']))
				{
					if( $index < $num_rows-1)
					{
						echo ";";
...
<input type="submit" name="action"  value="Test Connection"><br />
  ...
  <table border="0">
	...
	<tr>
      <td>Extended inserts: </td>
      <td><input type="checkbox" name="sql_extends_inserts"  <?php if(isset($_REQUEST['action']) && ! isset($_REQUEST['sql_extends_inserts'])) ; else echo 'checked' ?>/></td>
    </tr>
